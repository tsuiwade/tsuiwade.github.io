var gitee_canlendar = (git_user, git_color) => {
    var git_giteeapiurl = "https://python-gitee-calendar-api.vercel.app/api?" + git_user;
    var git_fixed = 'fixed';
    var git_px = 'px';
    ...
    fetch(git_giteeapiurl).then(data => data.json()).then(data => {
        ...
        addweek(data);
        addlastmonth();
        var html = gitee_main_box(git_monthchange, git_data, git_user, git_color, git_total, git_thisweekdatacore, git_weekdatacore, git_oneyearbeforeday, git_thisday, git_aweekago, git_amonthago);
        append_div_gitcalendar(gitee_container, html);
        if (document.getElementById('gitee_loading')) {
            document.getElementById('gitee_loading').remove()
        };
        responsiveChart()
    ...
    var gitee_main_box = (monthchange, git_data, user, color, total, thisweekdatacore, weekdatacore, oneyearbeforeday, thisday, aweekago, amonthago) => {
        var html = '';
        var canvasbox = gitee_canvas_box();
        var infobox = gitee_info_box(user, color);
        var style = gitee_main_style();
...
(function () {
    ...
    append_div_gitcalendar(gitee_container, loading_git(git_color[4]));
    gitee_canlendar(git_user, git_color)
})()
